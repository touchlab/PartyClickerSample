...
- (jint)countCurrentPartyWithInt:(jint)partyId {
  id<AndroidDatabaseCursor> cursor = [((id<OrgGreenrobotGreendaoDatabaseDatabase>) nil_chk([((PDDaoSession *) nil_chk(daoSession_)) getDatabase])) rawQueryWithNSString:@"select sum(val) from person where party_Id = ?" withNSStringArray:[IOSObjectArray arrayWithObjects:(id[]){ JavaLangInteger_toStringWithInt_(partyId) } count:1 type:NSString_class_()]];
  [((id<AndroidDatabaseCursor>) nil_chk(cursor)) moveToFirst];
  return [cursor getIntWithInt:[cursor getColumnIndexWithNSString:@"sum(val)"]];
}
