//
//  Generated by the J2ObjC translator.  DO NOT EDIT!
//

#include "J2ObjC_source.h"
#include "PDParty.h"
#include "java/lang/Long.h"
#include "java/text/DateFormat.h"
#include "java/text/SimpleDateFormat.h"
#include "java/util/Date.h"

inline JavaTextSimpleDateFormat *PDParty_get_timeFormat();
static JavaTextSimpleDateFormat *PDParty_timeFormat;
J2OBJC_STATIC_FIELD_OBJ_FINAL(PDParty, timeFormat, JavaTextSimpleDateFormat *)

inline JavaTextDateFormat *PDParty_get_standardDateFormat();
static JavaTextDateFormat *PDParty_standardDateFormat;
J2OBJC_STATIC_FIELD_OBJ_FINAL(PDParty, standardDateFormat, JavaTextDateFormat *)

inline JavaTextDateFormat *PDParty_get_standardTimeFormat();
static JavaTextDateFormat *PDParty_standardTimeFormat;
J2OBJC_STATIC_FIELD_OBJ_FINAL(PDParty, standardTimeFormat, JavaTextDateFormat *)

J2OBJC_INITIALIZED_DEFN(PDParty)

@implementation PDParty

- (instancetype)initWithJavaLangLong:(JavaLangLong *)id_
                        withNSString:(NSString *)name
                    withJavaUtilDate:(JavaUtilDate *)created {
  PDParty_initWithJavaLangLong_withNSString_withJavaUtilDate_(self, id_, name, created);
  return self;
}

J2OBJC_IGNORE_DESIGNATED_BEGIN
- (instancetype)init {
  PDParty_init(self);
  return self;
}
J2OBJC_IGNORE_DESIGNATED_END

- (NSString *)dateString {
  return JreStrcat("$$$", [((JavaTextDateFormat *) nil_chk(PDParty_standardDateFormat)) formatWithJavaUtilDate:created_], @" - ", [((JavaTextDateFormat *) nil_chk(PDParty_standardTimeFormat)) formatWithJavaUtilDate:created_]);
}

- (NSString *)description {
  return JreStrcat("$$$", name_, @" - ", [((JavaTextSimpleDateFormat *) nil_chk(PDParty_timeFormat)) formatWithJavaUtilDate:created_]);
}

- (JavaLangLong *)getId {
  return id__;
}

- (NSString *)getName {
  return name_;
}

- (JavaUtilDate *)getCreated {
  return created_;
}

- (void)setIdWithJavaLangLong:(JavaLangLong *)id_ {
  JreStrongAssign(&self->id__, id_);
}

- (void)setNameWithNSString:(NSString *)name {
  JreStrongAssign(&self->name_, name);
}

- (void)setCreatedWithJavaUtilDate:(JavaUtilDate *)created {
  JreStrongAssign(&self->created_, created);
}

- (void)dealloc {
  RELEASE_(id__);
  RELEASE_(name_);
  RELEASE_(created_);
  [super dealloc];
}

+ (const J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    { NULL, NULL, 0x1, -1, 0, -1, -1, -1, -1 },
    { NULL, NULL, 0x1, -1, -1, -1, -1, -1, -1 },
    { NULL, "LNSString;", 0x1, -1, -1, -1, -1, -1, -1 },
    { NULL, "LNSString;", 0x1, 1, -1, -1, -1, -1, -1 },
    { NULL, "LJavaLangLong;", 0x1, -1, -1, -1, -1, -1, -1 },
    { NULL, "LNSString;", 0x1, -1, -1, -1, -1, -1, -1 },
    { NULL, "LJavaUtilDate;", 0x1, -1, -1, -1, -1, -1, -1 },
    { NULL, "V", 0x1, 2, 3, -1, -1, -1, -1 },
    { NULL, "V", 0x1, 4, 5, -1, -1, -1, -1 },
    { NULL, "V", 0x1, 6, 7, -1, -1, -1, -1 },
  };
  #pragma clang diagnostic push
  #pragma clang diagnostic ignored "-Wobjc-multiple-method-names"
  methods[0].selector = @selector(initWithJavaLangLong:withNSString:withJavaUtilDate:);
  methods[1].selector = @selector(init);
  methods[2].selector = @selector(dateString);
  methods[3].selector = @selector(description);
  methods[4].selector = @selector(getId);
  methods[5].selector = @selector(getName);
  methods[6].selector = @selector(getCreated);
  methods[7].selector = @selector(setIdWithJavaLangLong:);
  methods[8].selector = @selector(setNameWithNSString:);
  methods[9].selector = @selector(setCreatedWithJavaUtilDate:);
  #pragma clang diagnostic pop
  static const J2ObjcFieldInfo fields[] = {
    { "timeFormat", "LJavaTextSimpleDateFormat;", .constantValue.asLong = 0, 0x1a, -1, 8, -1, -1 },
    { "standardDateFormat", "LJavaTextDateFormat;", .constantValue.asLong = 0, 0x1a, -1, 9, -1, -1 },
    { "standardTimeFormat", "LJavaTextDateFormat;", .constantValue.asLong = 0, 0x1a, -1, 10, -1, -1 },
    { "id__", "LJavaLangLong;", .constantValue.asLong = 0, 0x1, 11, -1, -1, -1 },
    { "name_", "LNSString;", .constantValue.asLong = 0, 0x1, -1, -1, -1, -1 },
    { "created_", "LJavaUtilDate;", .constantValue.asLong = 0, 0x1, -1, -1, -1, -1 },
  };
  static const void *ptrTable[] = { "LJavaLangLong;LNSString;LJavaUtilDate;", "toString", "setId", "LJavaLangLong;", "setName", "LNSString;", "setCreated", "LJavaUtilDate;", &PDParty_timeFormat, &PDParty_standardDateFormat, &PDParty_standardTimeFormat, "id" };
  static const J2ObjcClassInfo _PDParty = { "Party", "com.kgalligan.partyclicker.data", ptrTable, methods, fields, 7, 0x1, 10, 6, -1, -1, -1, -1, -1 };
  return &_PDParty;
}

+ (void)initialize {
  if (self == [PDParty class]) {
    JreStrongAssignAndConsume(&PDParty_timeFormat, new_JavaTextSimpleDateFormat_initWithNSString_(@"MM/dd/yyyy hh:MM a"));
    JreStrongAssign(&PDParty_standardDateFormat, JavaTextDateFormat_getDateInstanceWithInt_(JavaTextDateFormat_MEDIUM));
    JreStrongAssign(&PDParty_standardTimeFormat, JavaTextDateFormat_getTimeInstanceWithInt_(JavaTextDateFormat_SHORT));
    J2OBJC_SET_INITIALIZED(PDParty)
  }
}

@end

void PDParty_initWithJavaLangLong_withNSString_withJavaUtilDate_(PDParty *self, JavaLangLong *id_, NSString *name, JavaUtilDate *created) {
  NSObject_init(self);
  JreStrongAssign(&self->id__, id_);
  JreStrongAssign(&self->name_, name);
  JreStrongAssign(&self->created_, created);
}

PDParty *new_PDParty_initWithJavaLangLong_withNSString_withJavaUtilDate_(JavaLangLong *id_, NSString *name, JavaUtilDate *created) {
  J2OBJC_NEW_IMPL(PDParty, initWithJavaLangLong_withNSString_withJavaUtilDate_, id_, name, created)
}

PDParty *create_PDParty_initWithJavaLangLong_withNSString_withJavaUtilDate_(JavaLangLong *id_, NSString *name, JavaUtilDate *created) {
  J2OBJC_CREATE_IMPL(PDParty, initWithJavaLangLong_withNSString_withJavaUtilDate_, id_, name, created)
}

void PDParty_init(PDParty *self) {
  NSObject_init(self);
}

PDParty *new_PDParty_init() {
  J2OBJC_NEW_IMPL(PDParty, init)
}

PDParty *create_PDParty_init() {
  J2OBJC_CREATE_IMPL(PDParty, init)
}

J2OBJC_CLASS_TYPE_LITERAL_SOURCE(PDParty)
